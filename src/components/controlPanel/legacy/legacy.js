...
export default class Legacy extends Component {
  ...
  goToLegacyVersion(version) {
    ...
    const legacyVersion =
      packageJson.legacyVersions[packageJson.legacyVersions.indexOf(version)];
  ...
  render() {
    ...
    return (
      <div class="legacy">
        ...
        <p>
          Older version of VaporBoy. Should only be used to continue / finish
          progress made on ROMS that were started on a previous version of
          VaporBoy. These versions are no longer supported / maintained.{" "}
          <b>Click a version to get redirected to the legacy version.</b>
        </p>
        <ul>{legacyVersions}</ul>
      </div>
    );
  }
}
